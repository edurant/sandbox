...
def get_requirements(class_list, need_csc5610, need_mth5810):
    """
    Provide a reconciliation of how and whether the degree requirements are met.

    Inputs are dict of semester course plans and needed topic flags from admission decision.
    """
    ...
    electives = [
        "BUS6141"
    ]  # Approved electives that don't fit into course number logic
    for crs in requirements.copy():
        if crs.startswith("CSC5xxx"):
            for opt in planned:
                if (opt in electives) or (
                    opt.startswith(("BME", "CSC"))
                    and opt[3].isdigit()
                    and int(opt[3]) >= 5
                ):
                    reqs[crs] = opt
...
def extract_grad_plan(plan):
    ...
    grad_plan = plan[
        (plan["Number"].str[0] >= "5") & (plan["Number"].str[0] <= "9")
...
def summarize_student(args, df):
    ...
    if df.shape[0] != 1:
        print(
            f"{df.shape[0]} records matching {args.name}: {df['First Name'].tolist()}, exiting…"
        )
        return -1
    ...
    reqs = get_requirements(
        classes, record["CSC5610 Needed?"], record["MTH5810 Needed?"]
    )
    pprint.pprint(reqs, sort_dicts=False)

    print("\nSearching for STAT Plan:")
    plans = get_plans(
        "_".join([record.name, record["First Name"]])
    )  # restrict beyond args.name
    if plans.empty:
        print("None found")
    else:
...
def summarize_course(args, df):
    ...
    if results:
        ...
        seat_counts = (
            enrolled.groupby(["Year", "Semester"]).size().reset_index(name="Count")
        )
...
def summarize_term(args, df):
    ...
    grouped.to_excel(
        args.name + ".xlsx", index=False, sheet_name=args.name, freeze_panes=(1, 0)
    )
...
def main(args):
    ...
    boolean_fields = [
        "Early Entry Originally",
        "BS Complete?",
        "GPA < 3",
        "HasLinearAlgebra",
        "HasMultivariableCalculus",
        "CSC5120 Needed?",
        "CSC5610 Needed?",
    ]
...
if __name__ == "__main__":
    ...
    data_path = [
        "OneDrive - Milwaukee School of Engineering",
        "MSML Admin",
        "msml.xlsx",
    ]
