...
def main():
    """
    Main function to parse arguments, parse the webpage, and print the results
    """
    parser = argparse.ArgumentParser(description=__doc__,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-p', '--course_prefix', type=str, default='CSC', help='Course prefix')
    parser.add_argument('-c', '--cur_cat_oid', type=int, default=list(NAVOID.keys())[-1],
        help='cur_cat_oid')
    parser.add_argument('-n', '--navoid', type=int, default=-1, help='navoid, -1 to infer')
    args = parser.parse_args()

    base_url = 'https://catalog.msoe.edu/content.php'

    catalog_title, course_links = fetch_and_parse_url(base_url,
        args.course_prefix, args.cur_cat_oid, args.navoid)
